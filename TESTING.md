...
⚠️ **Секреты/идентификаторы не хранить в git**.  
Значения для теста (email / Google OAuth client id / client secret) держать локально вне репозитория (например в приватной заметке Obsidian или в password manager) и вставлять только в UI Obsidian.
