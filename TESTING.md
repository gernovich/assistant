...
## Тесткейс "Test Window Transport: диалог + обмен сообщениями"

**Цель**: проверить, что тестовый диалог открывается и транспорт доставляет сообщения в обе стороны.

- Собрать и установить плагин `npm run build && npm run install:obsidian`
- Очистить лог‑файл если он есть
- Запустить Obsidian с `--remote-debugging-port=9222`
- Пауза 2000 мс
- Открыть панель без палитры: выполнить `setViewState({ type: "test_transport_panel" })`
- Убедиться, что в текущей панели видны кнопки: `Open dialog`, `App one`
- Нажать `Open dialog`
- Пауза 100 мс
- Убедиться, что появилось второе окно (dialog) с кнопками `Dialog one` и `Close`
- В панели нажать `App one`
- Пауза 100 мс
- В диалоге нажать `Dialog one`
- Пауза 100 мс
- Проверить в лог‑файле запись `test_transport_dialog_opened`
- В диалоге нажать `Close`
- Пауза 100 мс
- Убедиться, что окно диалога исчезло
- Проверить в лог‑файле запись `test_transport_dialog_closed`
- Закрыть Obsidian

**Ожидаемый результат**:

- Панель и диалог открываются без ошибок
- Сообщения отображаются в логах панели и диалога (window ↔ host)
- В лог‑файле есть записи `test_transport_dialog_opened` и `test_transport_dialog_closed`
- В консоли приложения нет ошибок
- В консоли диалога нет транспорта
