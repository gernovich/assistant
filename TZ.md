...
- [+] Логи вынести в системную папку /home/gernovich/notes/.obsidian/plugins/assistant/logs немного скрыть их, это уменьшит шум илишние файлы в хранилище (настройка логов не нужна)
- [+] Нужно сделать ротацию логов, точнее ограничение хранения, наприсер 7 дней, это должно настраиваться.
- [+] Кэширование данных календаря: offline-first “stale режим” + persistent cache (после рестарта тоже показываем last-good)
- [+] Название встречи — человеческое; идентичность держим через `(calendar_id, event_id)`, а при изменении summary делаем безопасный rename (суффиксы/конфликты не ломают связь)
- [+] Офлайн режим (MVP): очередь изменений (outbox). Если не получилось записать в vault — действие кладём в очередь + пишем причину в лог; есть кнопка “Применить очередь”.
- [+] В css использовать цвета, отстопы и другие элементы с не конкретные значения в переченные обсидиана, это даст консистентность интерфейса.
- [+] Нужно проработать поля карточек, что хранить вообще, что отобразать в базе, каие представления есть в базе
...
- [ ] Сделать временную панель и окно отработки WindowTransport, и на нем отработать технологию.
- [ ] Добавить логирование в ключевые места (Лишние убрать)
- [ ] Логирование, установить необходимые сообщения и удалить лишние. Основная задача что если что то пошло не так можно было быстро сориентироваться где проблема и уже точечно залогировать.
...
- [ ] Для индикатора записи, сделать умный буффер. Принимает события с любой частотой, отдает данные с нужной. Если долго нет событий, берет последнее значение и уменьшает на 10%. Нужно для того если события отдаются редко, всеравно было видно что процесс записи идет. Улучшаемм UX.
