...
- [x] Для индикатора записи сделать умный нормализатор/буфер (RecordingVizNormalizer): принимает события с любой частотой, отдаёт с нужной; нормализует амплитуду; логирует статистику с заданной частотой; при тишине снижает значение и держит полосу на нуле. Компонент тестируемый, покрыт тестами, без вынесения настроек в конфиг.
  ...
  - [x] Отлаживаем Linux Native, сейчас индикатор пустой (устарело, отказываемся от ffmpeg)
...
- [x] Запись звука через [gst-kit](https://github.com/repugraf/gst-kit)
  - [x] Убираем в настройках ffmpeg (linux_native) и все связнное с ним
  - [x] Проверить что удалены политики связанные с ffmpeg и их тесты. Поискать мертвый код.
  - [x] Добавляем в настройки g_streamer (GStreamer) добавляем проверку зависимостей
  - [x] Обновить документаци про зависимости, в целом можно взять от сюда gst-kit
  - [x] В настройках определение устройство микрофона (ваш голос) авто или выбор из текущих + индикатор уровня для проверки + выбор фильтра на канал "none" | "normalize" | "voice"
  - [x] В настройках определение устройство монитора (вы слышите) авто или выбор из текущих + индикатор уровня для проверки + выбор фильтра на канал "none" | "normalize" | "voice"
  - [x] Добавляем бэкенд для gst-kit, в диалог передаем два уровня громкости, микрофона и монитора
  - [x] gst-kit Делаем общий слой, подключение библиотеки, определение устройств, функции и т.д. Что бы реиспользовать в Диктофоне и настройках.
  - [x] Пишем файл не в tmp а сразу в нужное место и с нужным именем (Касается и Electron Media Devices)
  - [x] Рефакторим Electron Media Devices отдает только один уровень записи, микрофон (желтым).
  - [x] В диалоге Диктофона на индикаторе рисуем два графика, микрофон (желтым) и монитор (синим)
  - [x] Удалить scripts/gst-stand.cjs
- [x] Чат/LLM в этом проекте не делаем — используем Infio-Copilot: `/home/gernovich/projects/_tests/infio-copilot`
- [x] Прорабатываем визуализацию, статус события это по сути css клас: Без ответа событие: без заливки. Принятое как сейчас с заливкой. Отклонено без заливки и название зачеркнуто. Может быть заливка в полосочку по диагонали (чредование есть фон как у принятого и нету фона как у отклоненного). Метку " • принято" или другую делать не надо.
- [x] Работа со статусом события, должно быть двух направленная. Если я в повестке события указал что приду или другие статусы, это для события обновлялось и в календаре и в карточе Встречи (Карточка встечи это проекция ближайшего события).
- [x] Работа с цветом календаря и цветом встреч

Цель итерации, запись и расшифровка переговоров.
- [x] Нужно проверить уровни индикатора, и сделать что бы работало идеально.
- [x] Если не работает авторизация, то в повестке плашка "Оффлайн режим: ...",  нужно как минимум для офлайн добавить кнопку переподключиться (outline мы уже где то в сообщениях кнопку делали), и различать офлайн и не работает токен. Если токен не работает то сообщение перейти в настройки и авторизиоваться заново, доавить кнопку открыть настройки (outline мы уже где то в сообщениях кнопку детали).


- [ ] Детектирование звука и запущенного софта (Zoom) и всплывание окна диктофона, если понятно что это встреча, то обратный отчет с авто записью. (В конфиге авто запись и кол-во секунд по умолчанию включено и 5 секунд)
- [ ] В диктофоне при записи встречи нужно показывать текущее время, сколько прошло, сколько осталось встречи.
- [ ] В диктофоне при авто старте нужна кнопка остановить обратный отсчет, крестик после времени.
- [ ] Ложный запуск если меня нет за компом
...
- [ ] Транскрибация разговоров
  - [ ] Локальный сервис Whisper
  - [ ] Внешний сервис nexara.ru

- [ ] Отпечаток голоса
- [ ] Определение спикеров по отпечаткам
- [ ] Диаризация спикеров

- [ ] Проработать работу LLM и чатика
