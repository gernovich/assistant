...
Рефакторин и добить максимально повестку и календарь

- [x] Логи вынести в системную папку /home/gernovich/notes/.obsidian/plugins/assistant/logs немного скрыть их, это уменьшит шум илишние файлы в хранилище (настройка логов не нужна)
- [x] Нужно сделать ротацию логов, точнее ограничение хранения, наприсер 7 дней, это должно настраиваться.
...
- [x] Нужно проработать поля карточек, что хранить вообще, что отобразать в базе, каие представления есть в базе
- [x] Из календаря, извлечение информации об участниках в карточки
- [x] Отмечать встречи Да (Да в переговорке, да виртуально) / Нет / Может быть / Нет выбора (Одно или и последующие события)
...
- [x] Алерт напоминание сделать красивым через electron_window как окно настроект обсидиана что бы не смотрелось костылем. Можно добавить чуть чуть прозразчности.
- [x] При любом всплывающем алерте, не должно быть фокуса на клавишах, иначе если в момент печататья он окроится можно случайно быстро его закрыть
- [x] Кнопка записи разговора, на панели
- [x] Диалог записи разговора: 1. выбор события, по умолчанию если есть событие менее 5 минут (первое напоминание) то оно выбрано, или уже идет, иначе галочка новый протокол. 2. Круглая зеленая кнопка записать.
...
- [x] В диалоге напоминания,Создать протокол кнопка не нужна
- [x] В напоминании кнопкам [Начать запись], [Встреча отменена] и [Закрыть] добавть иконки и для закрыть красную outline рамку.
- [x] Если в настройках включена авто запись и указан тайминг (по умолчанию 5 секунд), в диалоге записи если встреча идет, то у казываем что авто запись начнется и обратный отсчет.
- [x] Окно диктофона или напоминания должны перетаскиваться за заголовок
- [x] Запись звука, продумать по хорошему сделать отдельными файлами по N минут, что бы уже во время встречи получать рассшифровку, факты, проекты и т.д. Главное не дожидаться окончания.
...
- [ ] Для индикатора записи, сделать умный "буффер". Принимает события с любой частотой, отдает данные с нужной. также нормализируе амплитуду что бы она хорошо смотрелась в индикаторе. Кажется нужно сделать логирование, но с определнной частотой которую можно задать, например раз 1 секунду или потом раз в минуту, в лог должно попадать как буфер отработал за это время, сколько пришло событий максимум минимум сколько ушло. Если нет событий, берет последнее значение и уменьшает на 10% (стратегии могут быть разные). Нужно для того, если события приходят редко, все равно было видно что процесс записи идет. Улучшаемм UX. Если звука нет то долна бежать полоса на нуле. Еще важное требование буффер должен быть тестируемый и покрыт тестами. Настройки буфера не выносим в конфиг это просто свойства буффера. И да придумай как назвать эту штуку, буффер мне не нравится.
  - [ ] Отлаживаем Electrom Media Devices, сейчас индикатор путой
  - [ ] Отлаживаем Linux Native, сейчас индикатор путой
- [ ] Транспорт диалогов, временно WebSocketTransport, перейти на MessageChannelTransport, по коду использовать только абстракцию WindowTransport
- [ ] В карточке встречи добавить участников
...
- [ ] Отработать механиз записи, в первую очередь довести ffmpeg до ума а потом попробывать поискать nodejs решение, может есть такая библиотека. Да резение на node js может запускать отдельным процессом и общаться через WindowTransport с основным процессом.

- [ ] Ревью всего проекта и особенно записи звука, повестки, настроек, dto, поиск точек улучшения

1. Transport: WS → MessageChannel (3–5 дн.)
   переключение реализации внутри транспортов без изменения UI/контроллеров
   smoke‑тесты: reminder/recording/test dialog
2. ✅ Логирование и диагностика (2–3 дн.)
   убрать шум, оставить ключевые цепочки
   единые префиксы + каналы (как для test_transport)
3. ✅ Индикатор записи (2–3 дн.)
   умный буффер (как в TZ.md)
   сглаживание + стабильный UX
4. Event vs Occurrence: жесткий контракт (3–4 дн.)
   проверка use‑case’ов, исключить смешение ключей
   минимальные тесты на контракт
5. Стабилизация диктофона Linux Native (4–6 дн.)
   ffmpeg пайплайн и устойчивость
   проверка записи/stop/flush
