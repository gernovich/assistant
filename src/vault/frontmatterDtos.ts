...
import { splitFrontmatter, parseFrontmatterMap } from "./frontmatter";
import { FM } from "./frontmatterKeys";

/**
 * Типизированные парсеры frontmatter -> DTO.
 *
 * Важно:
 * - `parseFrontmatterMap` читает только плоские `key: value` строки.
 * - Для списков/объектов используем `metadataCache` (отдельные парсеры можно добавить позже).
 */
...
/** Парсер из `metadataCache.frontmatter` для карточки протокола (поддерживает массивы/объекты). */
export function parseProtocolNoteFromCache(frontmatter: Record<string, unknown>): ProtocolNote {
  ...
  // “богатые” поля (могут отсутствовать/быть не тем типом)
  ...
  const participants: PersonLinkDto[] | undefined = Array.isArray(participantsRaw)
    ? (participantsRaw as unknown[])
        ...
        .map((x) => {
          ...
          const personId =
            typeof o.person_id === "string" ? o.person_id : typeof o.id === "string" ? o.id : undefined;
          const displayName =
  ...
  const projects: ProjectLinkDto[] | undefined = Array.isArray(projectsRaw)
    ? (projectsRaw as unknown[])
        ...
        .map((x) => {
          const o = x as Record<string, unknown>;
          const projectId =
            typeof o.project_id === "string" ? o.project_id : typeof o.id === "string" ? o.id : undefined;
...
/** Парсер из `metadataCache.frontmatter` для карточки человека. */
export function parsePersonNoteFromCache(frontmatter: Record<string, unknown>): PersonNoteDto {
  ...
  return {
    ...
    // messengers — объектный массив, зависит от Obsidian YAML parser; читаем как есть.
    messengers: Array.isArray(frontmatter[FM.messengers]) ? (frontmatter[FM.messengers] as any) : undefined,
  };
}

/** Парсер из `metadataCache.frontmatter` для карточки проекта. */
