/**
 * Простейшие утилиты для работы с YAML frontmatter в md-файлах.
 *
 * Ограничения:
 * - рассчитано на “плоские” ключи `key: value`
 * - не парсит сложные структуры YAML
 *
 * Для наших задач (метки/статусы/ссылки в “видимой базе”) этого достаточно.
 */

type FrontmatterSplit = { frontmatter: string | null; body: string };

/** Разделить md на frontmatter и тело. */
...
/** Распарсить frontmatter в map `key -> value` (как строка после `:`). */
...
/** Сериализовать map `key -> value` в frontmatter (без `---` границ). */
...
/**
 * Обновить/удалить ключи в frontmatter.
 *
 * - `updates[key] = "value"` → поставить/обновить
 * - `updates[key] = null` → удалить ключ
 */
