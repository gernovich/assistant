...
export function redactSecretsInStringForLog(input: string): string {
  ...
  // Важно: не маскируем произвольные `#...` в тексте, потому что это ломает полезные данные (например hex-цвета `#RRGGBB`).
  // URL-фрагменты маскируются в `redactUrlForLog()`, который применяется отдельно для строк, похожих на URL.
  return out;
}
