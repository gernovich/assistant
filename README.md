...
### Транскрибация (ASR) через Nexara

Используем внешний сервис Nexara: [документация (quickstart)](https://docs.nexara.ru/ru/quickstart).

Минимальный режим:
- **без диаризации**
- **без нормализации**
- **с таймингами по сегментам** (используем `response_format=verbose_json` + `timestamp_granularities[]=segment`)

Настройка: Settings → Ассистент → **Транскрибация (Nexara)**:
- `enabled`: включить фоновые задачи
- `service`: `nexara`
- `token`: Bearer token (хранится в `.obsidian/plugins/assistant/data.json`)
- `pollMinutes`: раз в N минут искать “не расшифровано” и добавлять в протокол
...
- `Ассистент: Диктофон`
- `Ассистент: Транскрибация: запустить сейчас`
- `Ассистент: Обновить календари`
- `Ассистент: Создать карточку встречи`
- `Ассистент: Создать карточку протокола`
- `Ассистент: Создать протокол из открытой карточки`
- `Ассистент: Создать карточку человека`
- `Ассистент: Создать карточку проекта`
- `Ассистент: Создать карточки людей из участников`
- `Ассистент: Применить офлайн-очередь`
- `Ассистент: Принято (в календаре, из заметки встречи)`
- `Ассистент: Отклонено (в календаре, из заметки встречи)`
- `Ассистент: Возможно (в календаре, из заметки встречи)`
- `Ассистент: Нет ответа (в календаре, из заметки встречи)`
- `Ассистент: Применить статус из заметки в календарь`
- `Ассистент: Создать протокол из текущей встречи`
...
В этом репозитории **чат не реализуем** (код удалён).

За основу берём **Infio-Copilot**: `/home/gernovich/projects/_tests/infio-copilot`

- Он уже содержит готовый “Cursor-like” UX: чат + apply/diff, автокомплит, RAG по vault, workspaces, insights, dataview, MCP.
- Дальше этот проект (“Assistant”) фокусируется на **календаре/повестке/диктофоне/протоколах** и инфраструктуре вокруг встреч.
