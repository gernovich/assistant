...
import { pickDefaultRecordingTargetPolicy } from "../../src/domain/policies/recordingTarget";
...
describe("domain/policies/recordingTarget", () => {
  it("selects ongoing event over soon event", () => {
    ...
    const r = pickDefaultRecordingTargetPolicy(events, now, 5, (c, id) => `${c}:${id}`);
    expect(r.createNewProtocol).toBe(false);
    expect(r.selectedEventKey).toBe("c:on");
  });
  ...
  it("selects soon event within 5 minutes", () => {
    ...
    const r = pickDefaultRecordingTargetPolicy(events, now, 5, (c, id) => `${c}:${id}`);
    expect(r.createNewProtocol).toBe(false);
    expect(r.selectedEventKey).toBe("c:soon");
  });
